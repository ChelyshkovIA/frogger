<?php
    ...
    if(isset($_COOKIE['user']) && isset($_COOKIE['hash'])) {
        ...
        if(count($res) == 1) {
            $getChatsQuery = 
           "SELECT Chats.Id, Users.Name, Users.Surname, Users.Login, Messages.Text, Messages.DateTime, UsersFrom.Name AS UserFromName, UsersFrom.Surname AS UserFromSurname 
            FROM Users
            ...
            where Users.Login = '$user' AND Users.Hash = '$hash' AND Messages.Id in (
                ...
                where users.id = 15 group by Chats.Id
            ) group by Chats.Id";
